...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
	static DHT sensor = {GPIOB, GPIO_PIN_5};
  /* USER CODE END 2 */
    ...
    uint8_t buffer = USART2->DR;
		...
		switch (buffer) {
			...
			case 49:
				...
				sprintf(message, "Temp: %dC\r\n", (uint8_t) data.temp);
				HAL_UART_Transmit(&huart2, (uint8_t*) message, strlen(message), 0xFF);
			...
			case 51:
				...
				sprintf(message, "RGB initialize\r\n");
				HAL_UART_Transmit(&huart2, (uint8_t*) message, strlen(message), 0xFF);
			...
			case 52:
				...
				sprintf(message, "RGB deinitialize\r\n");
				HAL_UART_Transmit(&huart2, (uint8_t*) message, strlen(message), 0xFF);
...
static void MX_TIM1_Init(void)
{
  ...
  sConfigOC.Pulse = TIMER_PERIOD / 8;
...
static void MX_TIM4_Init(void)
{
  ...
  sConfigOC.OCMode = TIM_OCMODE_PWM1;
  sConfigOC.Pulse = TIMER_PERIOD / 4;
  sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
